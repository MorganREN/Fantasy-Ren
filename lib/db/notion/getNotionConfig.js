...
export function parseConfigFromPage(pageRecordMap, content) {
  ...
  const configTableId = content.find(contentId => {
    ...
    return isConfigTableBlock(blockItem)
  })
...
/**
 * Notion represents inline and full-page databases with different block types.
 * Both are valid configuration tables and are handled identically below.
 */
export function isConfigTableBlock(blockItem) {
  const type = normalizePageBlock(blockItem)?.type
