...
import { resolveSupportedLocale } from '@/lib/i18n/routing'
...
export const redirectUserLang = pageId => {
  ...
  const supportedLocales = siteIds.map(extractLangPrefix).filter(Boolean)
  const targetLocale = resolveSupportedLocale(userLang, supportedLocales)
